...
**6-company-intro와의 차이점**: 홈페이지 시스템은 REST API 기반으로 전환되었으므로 JSP가 없으며 JAR 패키징을 사용한다. JDOM2, XMLBeans, Gson, commons-compress, commons-math3, Lucy XSS Servlet 등의 라이브러리가 추가 검증 대상이다.
...
| View | **REST (JSON)** | JSP 없는 REST API 기반으로 전환. @RestController 사용 |
| 패키징 | JAR | JSP 미사용으로 JAR 패키징 가능 |
...
4. **REST 전환**: 홈페이지 시스템은 REST API 기반으로 전환하여 JSP를 제거한다.
...
### 5.1 REST 기반 전환 (JSP 미사용)

홈페이지 시스템은 REST API 기반으로 전환하였으므로 JSP가 없다. 따라서 `war` 패키징 대신 `jar` 패키징을 사용하며, `tomcat-embed-jasper`와 JSTL이 불필요하다.
...
└── src/
    ...
    │   │   ├── controller/
    │   │   │   └── HealthCheckController.java  # REST 엔드포인트 (/health, /)
    │   │   ├── entity/
